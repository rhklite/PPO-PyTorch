...
class Agent(mp.Process):
    ...
    def __init__(self, name, memory, pipe, env_name, max_timestep,
    ...
    def run(self):
        ...
        while True:
            ...
            # take this many steps until update
            for i in range(self.update_timestep):
                ...
                # indicates end of an episode
                if done or (i % self.max_timestep) == 0:
                    episode_count += 1
                    state = self.env.reset()
            ...
            self.pipe.send(training_reward[-1])
            # self.pipe.send(sum(training_reward)/len(training_reward))
            recieved = self.pipe.recv()

            # process recieved instructions
            if recieved == "END":
                print("Agent {} has stopped".format(self.name))
                break
            elif recieved == "RENDER":
                self.render = True
            elif recieved == "NO_RENDER":
                self.render = False
            else:
                pass
...
def main():
    ...
    max_iteration = 50      # num of update iterations before stop training
    seed = None               # seeding the environment
    render = False
    render_delay = 0.02       # sleep this long to delay render speed
    ...
    for agent_id in range(num_agents):
        ...
        agent = Agent(str(agent_id), memory, p_end, env_name, max_timestep,
                      update_timestep, gamma, seed, render, render_delay)
        agents.append(agent)
    ...
    # add extra cycle for sending shutdown signal to subprocess
    for i in range(max_iteration+1):
        ...
        if i == max_iteration:
            msg = "END"
        # elif i == max_iteration - 2:
        #     msg = "RENDER"
        else:
            msg = i
            print("Update Iteration: {}, Reward: {}"
                  .format(i, int(sum(agent_rewards)/len(agent_rewards))))
            ppo.update(memory)
